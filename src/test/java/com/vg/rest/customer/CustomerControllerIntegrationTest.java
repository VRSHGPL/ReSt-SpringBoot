...
public class CustomerControllerIntegrationTest {
    ...
    public void setUp() throws Exception {
        ...
        /** fix json issue with date TODO **/

        customer = new Customer(Long
                .valueOf(1l), "Jane", "Doe", null, new Address("homeAdd", "offAdd", "mail@dom.com"));

        modifiedCustomer = new Customer(Long
                .valueOf(1l), "John", "Doe", null, new Address("homeAdd", "offAdd", "mail@dom.com"));
    }

    @Test
    public void whenValidInput_thenCRUD() throws IOException, Exception {
        mvc.perform(post("/v1/customers").contentType(MediaType.APPLICATION_JSON_VALUE + ";charset=UTF-8")
                .content(toJson(customer))).andExpect(status().isCreated());
        ...
        this.mvc.perform(
                put("/v1/customers").contentType(MediaType.APPLICATION_JSON_VALUE).content(toJson(modifiedCustomer)))
        ...
        this.mvc.perform(delete("/v1/customers/{customerId}", Long.valueOf(1l))).andExpect(status().isNoContent());

        customer = customerService.getCustomerById(1l);
        assertEquals(false, customer.isPresent());

        this.mvc.perform(get("/v1/customers/1").accept(MediaType.ALL))
